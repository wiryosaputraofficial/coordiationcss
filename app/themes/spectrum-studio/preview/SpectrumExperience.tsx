...
const people = [
  ["/themes/spectrum-studio/person-01.avif", "Arka Wijaya", "Founder / Creative Direction"],
  ["/themes/spectrum-studio/person-02.avif", "Maya Adelia", "Strategy / Brand Systems"],
  ["/themes/spectrum-studio/person-03.avif", "Keiko Tan", "Design / Motion"],
] as const;
...
export default function SpectrumExperience() {
  ...
  return <main className="spectrum-studio co-overflow-hidden" id="top">
    ...
    <section className="sp-team"><SectionHeading index="06" eyebrow="The studio">The people you meet<br /><em>are the people doing the work.</em></SectionHeading><div className="sp-team-grid co-grid">{people.map(([image,name,role])=><article className="sp-person sp-reveal" key={name}><div className="sp-person-image co-relative co-overflow-hidden"><img className="co-w-full co-h-full co-object-cover" src={image} alt={`Portrait of ${name}`} /><span className="co-inline-flex co-items-center">Studio profile <ThemeIcon name="arrow-right" size={14} tone="ink" /></span></div><div className="co-flex co-justify-between"><h3>{name}</h3><p>{role}</p></div></article>)}</div></section>
    ...
    <section className="sp-contact sp-paper co-relative co-overflow-hidden" id="contact"><div className="sp-contact-copy sp-reveal"><p>Have a project in mind?</p><h2>LET&apos;S MAKE<br />IT <span>MATTER.</span></h2></div><RoundLink href="mailto:hello@spectrum.studio">Start a project</RoundLink></section>
