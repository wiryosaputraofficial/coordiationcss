  ...
  return (
    <main className="components-page">
      <header className="site-header">
        ...
        <nav aria-label="Main navigation"><Link href="/">Home</Link><Link href="/icons">Icons</Link><Link href="/docs/components">Component guide</Link><Link href="/release-check">Release Check</Link></nav>
        <a className="header-cta" href="#component-catalog">Browse components <span aria-hidden="true">↓</span></a>
      </header>

      <section className="components-hero">
        <div className="components-hero-copy"><p className="component-overline">OPEN CODE · CO-UTILITIES · AI-READABLE</p><h1>Components you<br /><em>actually own.</em></h1><p>Install accessible React source directly into your project. Every component is monochrome by default, composed from Coordiation utilities, and yours to change.</p><div><a href="#component-catalog">Browse {registry.componentCount} components →</a><Link href="/docs/components">Read the architecture</Link></div></div>
        <div className="components-hero-demo" aria-label="Component installation example"><div className="component-terminal-top"><span>registry install</span><i>ready</i></div><pre><code><span>$</span> npx shadcn@latest add{`\n`}  https://your-domain.com/r/button.json</code></pre><div className="component-terminal-result"><b>✓</b><span>components/ui/button.tsx</span><em>owned source</em></div></div>
      </section>
