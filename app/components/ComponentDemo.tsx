...
export default function ComponentDemo({ name, title }: ComponentDemoProps) {
  ...
  switch (name) {
    ...
    case "attachment":
      return <div className="demo-attachment"><span aria-hidden="true">↗</span><div><b>brand-guideline.pdf</b><small>2.4 MB · PDF</small></div><button type="button" aria-label="Remove attachment">×</button></div>;
    case "breadcrumb":
    ...
    case "calendar":
      return <div className="demo-calendar-full"><header><button type="button" aria-label="Previous month">‹</button><b>August 2026</b><button type="button" aria-label="Next month">›</button></header><div><span>M</span><span>T</span><span>W</span><span>T</span><span>F</span><span>S</span><span>S</span>{[24, 25, 26, 27, 28, 29, 30].map((day) => <button type="button" className={day === 29 ? "active" : ""} key={day}>{day}</button>)}</div></div>;
    case "carousel":
      return <div className="demo-carousel"><button type="button" aria-label="Previous slide" onClick={() => setSlide(slide === 1 ? 3 : slide - 1)}>‹</button><div><b>0{slide}</b><span>Component story</span></div><button type="button" aria-label="Next slide" onClick={() => setSlide(slide === 3 ? 1 : slide + 1)}>›</button></div>;
    case "chart":
    ...
    case "collapsible":
      return <details className="demo-collapsible" open><summary><span>Framework settings</span><b>⌄</b></summary><p>Prefix: <code>co-</code></p></details>;
    case "combobox":
    ...
    case "dropdown-menu":
      return <details className="demo-menu" open><summary>Actions <span>⌄</span></summary><div role="menu"><button type="button" role="menuitem">Edit project</button><button type="button" role="menuitem">Make a copy</button><hr /><button type="button" role="menuitem">Archive</button></div></details>;
    case "empty":
      return <div className="demo-empty"><span>□</span><b>No projects yet</b><p>Create your first project to get started.</p><button type="button">New project</button></div>;
    case "field":
    ...
    case "item":
      return <div className="demo-item"><span>CO</span><div><b>Coordiation Core</b><small>Updated 2 minutes ago</small></div><button type="button" aria-label="More actions">•••</button></div>;
    case "kbd":
    ...
    case "pagination":
      return <nav className="demo-pagination" aria-label="Demo pagination"><button type="button" aria-label="Previous page">←</button><button type="button">1</button><button type="button" className="active" aria-current="page">2</button><button type="button">3</button><button type="button" aria-label="Next page">→</button></nav>;
    case "popover":
    ...
    case "questionnaire":
      return <fieldset className="demo-questionnaire"><legend>How will you use Coordiation?</legend><label><input type="radio" name="questionnaire-demo" defaultChecked /> Product app</label><label><input type="radio" name="questionnaire-demo" /> Design system</label><button type="button">Continue →</button></fieldset>;
    case "radio-group":
    ...
    case "select":
      return <label className="demo-select"><span>Component</span><select defaultValue="dialog"><option value="dialog">Dialog</option><option value="button">Button</option><option value="tabs">Tabs</option></select><i>⌄</i></label>;
    case "sheet":
      return <div className="demo-sheet"><main><span>Page content</span></main><aside><button type="button" aria-label="Close sheet">×</button><b>Edit profile</b><p>Update your public information.</p><label>Name<input defaultValue="Coordiation" /></label></aside></div>;
    case "sidebar":
      return <div className="demo-sidebar"><aside><b>CO</b><nav><button type="button" className="active">⌂ <span>Overview</span></button><button type="button">□ <span>Components</span></button><button type="button">⚙ <span>Settings</span></button></nav></aside><main><span>Workspace</span><b>Overview</b></main></div>;
    case "slider":
      return <label className="demo-slider"><span>Border radius <b>12px</b></span><input type="range" min="0" max="24" defaultValue="12" /></label>;
    case "toast":
      return <div className={`demo-toast${on ? "" : " is-hidden"}`} role="status"><span>✓</span><div><b>Deployment complete</b><small>Your site is now live.</small></div><button type="button" onClick={() => setOn(false)} aria-label="Dismiss notification">×</button></div>;
    case "toggle":
